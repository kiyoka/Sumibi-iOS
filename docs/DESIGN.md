...
- 空白・数字・記号を除いたASCII英字が10文字以下の場合は、通常の漢字変換、全ひらがな、全カタカナの順で3候補を返す
- ASCII英字が11文字以上の場合は長文とし、上記3候補に、ひらがな少なめ（漢字多め）、ひらがな多め（漢字少なめ）を加えた5候補を返す
- APIには候補配列を持つJSONだけを返すよう指定し、候補の順序を維持して表示する
- 重複候補と空の候補は除外する
